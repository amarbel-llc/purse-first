...
nix build .#mesa                     # mesa CLI (list-table renderer, RFC 0003)
...
In Nix, every Go binary is built via `mkGoModule` (defined in `gomod.nix`), a thin wrapper over `pkgs.buildGoApplication` from the gomod2nix overlay. `gomod.nix` also publishes RFC 0001 dual outputs `packages.${system}.go-pkgs` and `packages.${system}.go-pkgs-test` via `pkgs.mkGoPkgs`; self-consumption uses `go-pkgs-test` so each binary's `checkPhase` exercises the same artifact downstream consumers receive. The shared `gomod2nix.toml` lockfile at the workspace root pins external modules — local code changes never invalidate it. The lockfile is generated by [amarbel-llc/gomod2nix](https://code.linenisgreat.com/gomod2nix) (a fork of `nix-community/gomod2nix` with `go.work` support). Run `just build-nix-gomod2nix` after any `go.mod`/`go.sum`/`go.work` change; CI fails on drift via `git diff --exit-code -- gomod2nix.toml`.
