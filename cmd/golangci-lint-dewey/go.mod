// Standalone module (NOT in go.work): the golangci-lint dependency
// closure belongs to this lint binary alone, vendored via this
// directory's gomod2nix.toml rather than the shared workspace lockfile.
// The golangci-lint version below is the single source of truth for the
// binary AND the plugin ABI; dewey itself resolves through the local
// replace so the plugin always compiles from the same tree.
//
// This module is dewey-owned but cannot live inside libs/dewey/: the
