...
`), 0o644); err != nil {
	...
	// Code-generated header is present (now carries the build-identity
	// stamp, purse-first#164).
	assertContains(tt, got, "Code generated by dagnabit (")
	assertContains(tt, got, "); DO NOT EDIT.")

	// External imports are preserved.
	assertContains(tt, got, `"fmt"`)
	assertContains(tt, got, `"strings"`)

	// Source declarations are copied verbatim, including function body
	// and unexported helper.
	assertContains(tt, got, "// Widget holds a name.")
	assertContains(tt, got, "type Widget struct{ Name string }")
	assertContains(tt, got, "// Greet returns")
	assertContains(tt, got, `return fmt.Sprintf("hi %s", strings.TrimSpace(name))`)
	assertContains(tt, got, "func unexportedHelper() int { return 42 }")

	// NO alias-style re-export in copy mode.
	assertNotContains(tt, got, "internal.Widget")
	assertNotContains(tt, got, "internal.Greet")
	assertNotContains(tt, got, `import internal "example.com/mod/internal/alfa/widget"`)
}
...
`), 0o644); err != nil {
	...
	assertContains(tt, got, `"example.com/mod/pkgs/shared"`)
	assertNotContains(tt, got, `"example.com/mod/internal/0/shared"`)
}
...
`), 0o644); err != nil {
	...
	assertContains(tt, string(debugOut), "type DebugWidget struct{}")
}
...
`), 0o644); err != nil {
	...
	assertContains(tt, string(out), `"example.com/mod/pkgs/shared"`)
}
