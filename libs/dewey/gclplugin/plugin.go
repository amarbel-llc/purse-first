...
// repool (enabled by default) plus actx (opt-in) — into golangci-lint's
// module plugin system so a downstream repo that already gates on
// golangci-lint can fold them into that same run instead of a separate
// `go vet -vettool` pass.
//
...
// packages that do not import the pool ownership types. actx is opt-in
// (disabled unless set to true) because enforcing dewey's ActiveContext
// over stdlib context.Context is an ecosystem-specific opinion.
//
...
func (p *Plugin) BuildAnalyzers() ([]*analysis.Analyzer, error) {
	analyzers := make([]*analysis.Analyzer, 0, 4)
