...
func TestInstallMCPWithHTTPURL(t *testing.T) {
	app := NewApp("remote", "Remote MCP server")
	app.MCPURL = "https://api.example.com/mcp"

	configPath := filepath.Join(t.TempDir(), ".claude.json")

	if err := app.installMCPTo("", configPath); err != nil {
		t.Fatalf("installMCPTo: %v", err)
	}

	config := readTestConfig(t, configPath)
	server := getTestServer(t, config, "remote")

	assertField(t, server, "type", "http")
	assertField(t, server, "url", "https://api.example.com/mcp")

	if _, ok := server["command"]; ok {
		t.Error("http entry should not have command field")
	}
	if _, ok := server["args"]; ok {
		t.Error("http entry should not have args field")
	}
}

func TestInstallMCPWithHTTPHeaders(t *testing.T) {
	app := NewApp("authed", "Authenticated MCP server")
	app.MCPURL = "https://api.example.com/mcp"
	...
	config := readTestConfig(t, configPath)
	server := getTestServer(t, config, "authed")
	...
	if got, _ := headers["Authorization"].(string); got != "Bearer tok-123" {
		t.Errorf("Authorization = %q, want %q", got, "Bearer tok-123")
	}
}

func TestInstallMCPHTTPWithoutHeaders(t *testing.T) {
	app := NewApp("noauth", "No-auth MCP server")
	app.MCPURL = "http://localhost:8080/mcp"
	...
	config := readTestConfig(t, configPath)
	server := getTestServer(t, config, "noauth")

	if _, ok := server["headers"]; ok {
		t.Error("headers should be omitted when MCPHeaders is empty")
	}
}

func TestInstallMCPHTTPPreservesStdioEntries(t *testing.T) {
	configPath := filepath.Join(t.TempDir(), ".claude.json")
	...
	app := NewApp("remote", "Remote server")
	app.MCPURL = "https://api.example.com/mcp"
	...
	local := getTestServer(t, config, "local-server")
	assertField(t, local, "type", "stdio")
	assertField(t, local, "command", "/usr/bin/local")

	remote := getTestServer(t, config, "remote")
	assertField(t, remote, "type", "http")
	assertField(t, remote, "url", "https://api.example.com/mcp")
}
